...
def generate_gemini_analysis():
    ...
    try:
        ...
        if not gemini_analyzer:
            return jsonify({'error': 'Gemini AI not initialized'}), 500
    ...
    try:
        ...
        if not gemini_analyzer:
            ...
            return jsonify({
                ...
                'analysis': {
                    'overall_performance': {
                        'summary': 'Analysis is being processed. Please check back shortly for detailed insights.',
                        'score_percentage': user_data['test_results'].get('score_percentage', 0),
                        'performance_level': 'Good',
                        'rank_estimate': 'Analysis in progress'
                    },
                    'subject_analysis': user_data.get('subject_performance', {}),
                    'strengths_analysis': ['Keep practicing to identify your strengths'],
                    'improvement_areas': ['Focus on weak areas identified in your performance'],
                    'personalized_recommendations': {
                        'study_plan': 'Continue regular practice and focus on conceptual understanding',
                        'resource_suggestions': ['Review textbooks', 'Practice more questions', 'Take regular tests']
                    },
                    'time_analysis': {
                        'efficiency_insights': ['Monitor your time management during tests']
                    },
                    'motivational_insights': {
                        'progress_indicators': ['You are making progress with each test'],
                        'encouragement': 'Keep up the good work! Consistent practice leads to improvement.'
                    }
                }
            }), 200
        ...
        logger.info(f"Generated Gemini analysis for user {user_id}")
        ...
        return jsonify({
            ...
            'analysis': {
                'overall_performance': {
                    ...
                    'rank_estimate': 'Unable to estimate'
                }
            }
        }), 500
